<template>
	<div>
	  ...
	  <ThreeScene :onLiveUpdate="handleLiveUpdate" :initialSettings="settings" />
		...
		<div v-if="showSettingsPanel">
		  ...
		  <form @submit.prevent="handleUpdateSettings">
			<div>
			  ...
			  <input type="number" step="any" v-model="settings.rotation" id="rotation" />
			</div>
			<div>
  ...
  export default {
	...
	methods: {
	  ...
	  handleLiveUpdate(liveSettings) {
		// Update settings with live updates from ThreeScene
		if (liveSettings.rotation !== undefined) {
		  this.settings.rotation = liveSettings.rotation;
		}
		if (liveSettings.zoom !== undefined) {
		  this.settings.zoom = liveSettings.zoom;
		}
		if (liveSettings.panX !== undefined) {
		  this.settings.panX = liveSettings.panX;
		}
		if (liveSettings.panY !== undefined) {
		  this.settings.panY = liveSettings.panY;
		}
	  },
